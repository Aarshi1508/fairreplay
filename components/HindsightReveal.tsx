...
import type { Hindsight, DecisionAction } from "@/data/scenarios";
import type { DecisionResult } from "@/lib/decisionEngine";

const unknowns = [
  "Opponent's future response",
  "How the race would evolve after this point",
  "Whether the decision would actually succeed",
];

function qualityClasses(quality: Hindsight["decisionQuality"]) {
  if (quality === "GOOD") {
...
function contextLine(unfavorable: boolean, quality: Hindsight["decisionQuality"], action: DecisionAction) {
  if (unfavorable && quality === "GOOD") {
    return `The ${action.toLowerCase()} didn't come off. Given the information available at T, the model still favored ${
      action === "ATTACK" ? "attacking" : "saving"
    }. That's the point.`;
  }
  if (!unfavorable && quality === "GOOD") {
...
export default function HindsightReveal({
  hindsight,
  decision,
  revealed,
  onReveal,
  ...
  const decisionScore =
    decision.recommendation === "ATTACK" ? decision.attackScore : decision.saveScore;
  ...
  const unfavorable =
    hindsight.resultLabel.toUpperCase().includes("FAILED") ||
    hindsight.resultLabel.toUpperCase().includes("LOST");
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-5 rounded-sm border border-dashed border-base-line2 bg-base-panel px-6 py-16 text-center">
        ...
        <p className="max-w-sm text-sm text-ink">
          The future stays locked until you choose to look at it — same as it
          was for the driver.
        </p>
        <button
          onClick={onReveal}
          className="label-mono rounded-sm bg-signal-red px-6 py-3 text-xs text-base transition hover:bg-signal-red/90"
        >
