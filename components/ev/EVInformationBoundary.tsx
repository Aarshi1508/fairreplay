...
const hiddenItems = [
  ...
  "Future charger contention",
  "Other vehicles' future state",
  "Actual arrival/reserve outcome",
];

export default function EVInformationBoundary({ state }: { state: EVDecisionPointState }) {
  const availableItems = [
    `Energy state — ${state.energyPercent} (modelled)`,
    `Distance remaining — ${state.distanceRemainingKm} km`,
    `Traffic condition — ${state.trafficCondition.toLowerCase()}`,
    `Charger availability — ${state.chargerAvailability.toLowerCase()}`,
    `Demand next hour — ${state.estimatedDemandNextHour} (assumed)`,
    `Fleet vehicles below reserve — ${state.fleetVehiclesBelowReserve} of ${state.fleetSize}`,
  ];
